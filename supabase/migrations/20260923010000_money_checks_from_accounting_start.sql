...
-- Session 45 (Opus 5), D-219.1: money before the accounting start is historical (Lloyd 2026-09-23:
-- "consider it as historical at first and accept whatever discrepancies since we don't have an
-- established system yet"). The start is acct_settings.accounting_start, 2026-09-01 in production.
--
-- health_checks_core_v1 is restated in full. Its body was not retyped: a script cut it from
-- 20260921020000_health_checks_core.sql, asserted the cut hashes to the live md5(prosrc)
-- f169d017fa1a9a9d768dd495d7219caf (read 2026-09-23), then applied exact one-hit replacements:
--   completed_stays_paid  only stays checked out on or after the start
--   payout_totals_agree   stays on or after the start, THEIR linked payout e-mails, adjustments dated on or after it
--   ledger_position       opening cash + income - expenses - drawings since the start; a problem only below zero
-- Nothing else changes: the other eight checks, the grants and both wrappers are untouched.
--
-- Evidence the old findings were bookkeeping, not loss: Airbnb's transaction CSV reconciles to the centavo
-- (D-219). Read on production before this was written, from 2026-09-01: 0 unpaid stays; payouts 26,259.85
-- promised = 26,259.85 received; position 107,083.01 opening + 12,398.46 - 111,776.95 = 7,704.52.
