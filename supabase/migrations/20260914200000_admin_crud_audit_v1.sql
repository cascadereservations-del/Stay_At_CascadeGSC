...
returns jsonb language plpgsql security definer set search_path = '' as $$
declare v_today date; v_fin boolean; r record; v_out jsonb := '[]'::jsonb;
begin
  perform public.admin_require('read_operations', p_property_id);
  v_fin := public.current_staff_authorized('read_finance', p_property_id);
  v_today := public.manila_today();
  create temp table if not exists _hc (check_key text, label text, status text, count integer, detail jsonb) on commit drop;
  delete from _hc;

  -- Ledger ↔ reservations: every confirmed payout e-mail names its stay.
  insert into _hc select 'payout_rows_linked', 'Payout e-mails linked to a stay', case when count(*) = 0 then 'pass' else 'fail' end, count(*)::int,
  ...
  insert into _hc select 'completed_stays_paid', 'Completed stays with a payout row', case when count(*) = 0 then 'pass' else 'warn' end, count(*)::int,
  ...
  insert into _hc select 'payout_totals_agree', 'Reservation payouts equal payout e-mails plus adjustments', case when abs(d.diff) < 1 then 'pass' else 'warn' end, 0,
  ...
  insert into _hc select 'checkouts_cleaned', 'Checkouts (90 days) followed by a cleaning', case when count(*) = 0 then 'pass' else 'warn' end, count(*)::int,
  ...
  if v_fin then
    insert into _hc select 'cleaner_fees_settled', 'Cleaning fees settled in the ledger', case when count(*) = 0 then 'pass' else 'fail' end, count(*)::int,
  ...
  insert into _hc select 'meter_readings_reviewed', 'Odd meter readings reviewed', case when count(*) = 0 then 'pass' else 'warn' end, count(*)::int,
  ...
  insert into _hc select 'inventory_ledger_consistent', 'Inventory quantities agree with movements', case when count(*) = 0 then 'pass' else 'fail' end, count(*)::int,
  ...
  if v_fin then
    ...
    insert into _hc select 'ledger_duplicates', 'Duplicate ledger rows', case when count(*) = 0 then 'pass' else 'warn' end, count(*)::int,
    ...
    insert into _hc select 'ledger_position', 'Cash position: income − expenses − drawings', case when abs(p.net) < 1 then 'pass' else 'warn' end, 0,
    ...
    insert into _hc select 'journals_balanced', 'Posted journals balance', case when count(*) = 0 then 'pass' else 'fail' end, count(*)::int,
  ...
  for r in select * from _hc loop
    insert into public.admin_health_check_runs(property_id, check_key, label, status, count, detail, ran_at, ran_by)
    values (p_property_id, r.check_key, r.label, r.status, r.count, r.detail, now(), auth.uid())
