...
-- SPEC-11 session 1 step 3, the half of V10 that is not the refactor.
-- Depends on 20260921020000_health_checks_core.sql, which supplies
-- run_health_checks_service_v1.
--
-- V10 turns each non-passing System health check into a finding keyed
-- V10:<check_key>, so the ten checks that only ever lived behind a dashboard
-- button now say something once a day without anyone pressing it.
--
-- TWO DEPARTURES FROM THE SPEC, both deliberate.
--
-- 1. V10 READS admin_health_check_runs; it does not call
--    run_health_checks_service_v1. The spec says "through
--    run_health_checks_service_v1", and that would work - a stable plpgsql
--    function calling a volatile writer was probed against this database on
--    2026-09-21 and did NOT raise. But the health checks persist their own
--    result, so the table already holds the answer, and calling the writer from
--    here would cost both of run_system_verifier_v1's promises: that it is
--    `stable`, and that ?dry=1 writes nothing at all. The Edge Function
--    refreshes first instead, which is one extra RPC on the daily scope.
-- 2. Because of 1, V10 carries its own staleness guard. If the newest run is
--    over 36 hours old, V10:stale fires. That guard exists precisely because
--    departure 1 invents a way for this check to go quietly wrong, and the
--    failure is not hypothetical: production's stored run was from 2026-09-14
--    when this was written, seven days stale, because nobody had pressed the
--    button since 20260916150000 added a check to it.
--
-- Both functions below are RESTATED IN FULL because create or replace has no
-- other form. Neither body was retyped: each was extracted from
-- 20260921010000_system_verifier_phase1.sql by line range and edited by sed,
-- and the diff against that file is in the session note. The edits are:
--   run_system_verifier_v1  + v_hc_ran declaration, + the V10 block
--   apply_verifier_run_v1   + V10 in the daily scope array,
--                           + insert an accepted finding as acknowledged

begin;

-- 1. The checks, with V10 added ---------------------------------------------
...
begin
  ...
  if v_daily then
    ...
    if v_hc_ran is null or v_hc_ran < p_now - interval '36 hours' then
      v_found := v_found || jsonb_build_array(jsonb_build_object(
        'key', 'V10:stale', 'check_id', 'V10', 'severity', 'yellow',
...
comment on function public.run_system_verifier_v1(uuid, text, timestamptz) is
  'SPEC-11 checks V1-V6, V10, V11, V12. Reads only, stores nothing, decides nothing about alerting. service_role only.';

-- 2. Alerting, with V10 in the daily scope and K16 born acknowledged --------
...
begin
  ...
  v_checks := case p_scope
    ...
    else                array['V1','V2','V3','V4','V5','V6','V10','V11','V12']
  end;
...
comment on function public.apply_verifier_run_v1(text, jsonb, timestamptz) is
  'Records a verifier run: what is new, what is due a reminder, what has gone. Performs the two safe auto-resolutions. Resolves only checks the given scope actually ran. An accepted finding (K16) is born acknowledged. service_role only.';
