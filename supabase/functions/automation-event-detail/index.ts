...
import { buildEventDetail, workflowAudience } from './logic.ts';
...
Deno.serve(async (request) => {
  ...
  const { data: event } = await db.from('automation_outbox').select('id,event_type,aggregate_id,status,payload,route_class,template_key').eq('id', body.event_id).maybeSingle();
  if (!event || !workflowAudience(event.event_type, body.workflow_id)) return json({ error: 'not_found' }, 404);
  ...
  const detail = buildEventDetail({ ...event, payload }, body.workflow_id, booking, expiresAt);
