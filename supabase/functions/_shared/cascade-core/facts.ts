...
// SPEC-13: a review number quoted in chat comes from this one constant, with the date it was read on the live listing
...
- Verified Airbnb property, ${AIRBNB_RATING.stars} stars, Guest Favorite. Tagline: Hotel Comfort, Home Warmth.
...
- Stays of 5+ nights: complimentary mid-stay room refresh (fresh linens, towels, toiletries).
...
BOOKING & PAYMENT
- Direct: ${SITE_URL} - the booking page carries live availability, full amenities, the exact total, and the payment options. In this chat, payment is GCash: the QR we send carries the exact amount. ${MAYA_FACT} When the guest asks for a bank, a UnionBank InstaPay transfer works too; the host sends those details by hand (D-247). The GCash number and QR are sent in this chat at the fee step, before the host's confirmation. Account details appear only inside the booking flow. Acknowledgement Receipt by email within 1-2 hours; dates are confirmed by the host's personal message. Refundable security deposit PHP 1,000, returned in full after a satisfactory check-out inspection. Full payment and the PHP 1,000 deposit at least a day before check-in; the door PIN follows full payment.
...
- Social proof once, only where it fits: verified ${AIRBNB_RATING.stars}-star Airbnb Guest Favorite.
