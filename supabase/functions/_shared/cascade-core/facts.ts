...
BOOKING & PAYMENT
- Direct: ${SITE_URL} - the booking page carries live availability, full amenities, the exact total, and the payment options. In this chat, payment is GCash: the QR we send carries the exact amount. Mention UnionBank transfer only when the guest asks about a bank or another way to pay (D-247). Account details appear only inside the booking flow. Acknowledgement Receipt by email within 1-2 hours; dates are confirmed by the host's personal message. Refundable security deposit PHP 1,000, returned in full after a satisfactory check-out inspection. Full payment on or before check-in; the door PIN follows full payment.
