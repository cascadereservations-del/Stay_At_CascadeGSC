-- Rollback for release verifier_triage_and_opening_amounts_20260923.
-- Run with scripts/migrations/run-sql-on-host.sh from stay-site. Restores both functions to the bodies
-- that were live before the release (cut from their applied migrations and hash-checked:
-- run_system_verifier_v1 178b72de3f830cc7a2ab7acd0d7711bb, approve_opening_balances_v1
-- c63e53a873439c1b7a5a95f7c060f88f), removes the heartbeat default, then drops the phrase function.
-- Stored verifier_findings keep their problem-phrase titles until the next daily run rewrites them.
-- The ledger row 20260923000000 stays; delete it by hand only if the migration is to be re-applied.
...
alter table public.job_heartbeats alter column last_succeeded_at drop default;

drop function if exists public.health_check_problem_v1(text);
