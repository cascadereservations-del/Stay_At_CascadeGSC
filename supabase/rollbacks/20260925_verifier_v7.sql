-- Rollback for release verifier_v7_20260925. Restores run_system_verifier_v1 (md5 12b080bf...) and
-- apply_verifier_run_v1 (md5 acc043e2...) to their pre-release bodies, cut from the migrations that applied
-- them, and drops telegram_answer_calendar_block_v1. Data written while the release was live stays:
-- reservation dates corrected from the calendar (they are the Airbnb calendar's dates), recon_status answers
-- and airbnb_unmatched tasks. Open V7/V7b/V13 findings are closed here: the old scope arrays omit those
-- checks, so no later run would ever resolve them.
begin;
...
drop function if exists public.telegram_answer_calendar_block_v1(bigint, text, text);

update public.verifier_findings
   set status = 'resolved', resolved_at = now(), resolved_by = 'rollback'
 where check_id in ('V7', 'V7b', 'V13') and status in ('open', 'acknowledged');

do $$
begin
  if (select md5(prosrc) from pg_proc where oid = 'public.run_system_verifier_v1(uuid, text, timestamptz)'::regprocedure) <> '12b080bfff25b908f20bf4ae3813af9e'
     or (select md5(prosrc) from pg_proc where oid = 'public.apply_verifier_run_v1(text, jsonb, timestamptz)'::regprocedure) <> 'acc043e2048f9d88e0109ce8d7200f36' then
    raise exception 'rollback bodies do not hash to the pre-release md5';
  end if;
end $$;
