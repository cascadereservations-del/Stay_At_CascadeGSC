...
| P3 — Dormant Cascade stack | The isolated stack exists with zero external business effect | Create only the Cascade stack, volumes, networks and secrets through Portainer CE; use an owner SSH tunnel to its loopback editor; start PostgreSQL then n8n; create the named owner and MFA; omit provider credentials; import all 13 workflows inactive | Both services healthy and inside caps; editor access protected by SSH plus n8n MFA; all workflow exports inactive and hash-matched; no DNS/proxy route exists | Yes; stop for action-time approval |
