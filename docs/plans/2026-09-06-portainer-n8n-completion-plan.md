...
| P4 — Recovery drill and 72-hour soak | **Next; not started** | Run encrypted backup and disposable restore of the new stack; observe host/container memory, swap, load, disk, restarts, health, and logs for at least 72 hours with workflows inactive | Restore proof plus soak report with no abort condition | Fresh approval for backup/restore; observation read-only after approved start |
