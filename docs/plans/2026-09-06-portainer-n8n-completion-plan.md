...
| P5 — Module A production closure | **Gate 1 BLOCKED (Supabase auth), deferred 2026-09-07; gates 2–6 not started** | Prove encrypted Supabase backup/restore; reconcile migration ledger; release staff/RLS/named-cleaner changes through the cutover packet; enroll project-owner TOTP and prove fresh AAL2; assign and smoke-test a real cleaner; release the corrected price-history view; configure scheduler/liveness only after its own review | Every Module A gate has dated evidence and rollback result | Yes; each production batch requires approval |
