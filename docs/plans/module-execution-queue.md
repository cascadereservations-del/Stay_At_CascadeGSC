...
**Progress (verified 2026-08-31):** named staff plus authenticated cleaner/RLS release candidate is complete and locally verified. Privacy enforcement is now a local release candidate: private request/hold/audit tables, AAL2 owner/admin RPCs, hold-blocked deletion transitions, a 49-assertion pgTAP contract and an expand-phase release contract are present. Observability utilities exist but still require staged adoption. Safe-release tooling binds ordered migrations to SHA-256, rejects destructive expand SQL, verifies source ancestry/local ledger state and documents non-destructive rollback. Wave 0.8 recovery remains proven; with privacy included, a disposable Supabase project applies all 28 recovery migrations and passes all 15 database test files without altering the active local stack. Production staff/privacy activation and restoration of the shared Portainer n8n data/credentials/encryption key remain open; Module B must not start before those Module A gates are closed or explicitly re-sequenced.
